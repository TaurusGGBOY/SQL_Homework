...
public class ReferInfo3 extends JPanel {
	...
	public ReferInfo3() {
		...
		for (Map<String, Object> info : btList) {
			btMap.put(String.valueOf(info.get("bookType")), String.valueOf(info.get("Code")));
		...
		bnList = new MySQLHelper().query("select * from basic4");
		...
		infoList = new MySQLHelper().query(
				"SELECT r.shum as shum,b1.bjsName as bjsName, b3.bookType as bookType, r.ID as ID, r.bookTypeID as btID, r.bjsNameID as bnID from refer3 r,basic4 b1, basic3 b3 where r.bjsNameID=b1.ID and r.bookTypeID=b3.Code and b1.inUse='是' order by r.ID");
			...
			public void actionPerformed(ActionEvent arg0) {
				if (!textField_ID.getText().equals("")) {
					...
					if (JOptionPane.showConfirmDialog(null, "删除该用户？" + textField_ID.getText().trim(), "提示",
						...
						new MySQLHelper()
								.executeNonquery("delete from refer3 where id = " + textField_ID.getText().trim());
		...
		btnNewButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if (!shumSet.contains(shum) && !idSet.contains(id)) {
					...
					System.out.println(bjsMap.get(name));
					new MySQLHelper().executeNonquery("Insert into refer3 (ID,bookTypeID,bjsNameID,shum) values (" + id
				...
				new MySQLHelper().executeNonquery("update refer3 set bjsNameID = " + bjsMap.get(name) + ",bookTypeID ='"
